...
class Ship :
    ...
    def draw(self) :
        pygame.draw.polygon(
            screen,
            WHITE,
            [(self.x, self.y), (self.x-20, self.y+30), (self.x+20, self.y+30)]
        )
        pygame.draw.polygon(
            screen,
            random.choice(FLAMES),
            [(self.x, self.y+30), (self.x-10, self.y+40), (self.x+10, self.y+40)]
        )
...
while running :
    ...
    if game_over :
        screen.blit(
            font.render('GAME OVER! Press R to Restart', True, WHITE),
